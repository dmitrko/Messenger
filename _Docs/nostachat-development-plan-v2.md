...
> **СТАТУС ПРОЕКТА (30.03.2026):**
> Развернута инфраструктура монорепозитория. Реализован базовый WebSocket-сервер и Desktop/Mobile клиенты (Hello World). Связь установлена.
> Текущий порт сервера: 3002. Текущий порт десктоп-клиента: 5173.
...
1. **Ностальгия** — дух ICQ/FidoNet, UIN без телефона, атмосфера раннего интернета
2. **Приватность** — E2E шифрование по умолчанию, пост-квантовый Kyber, P2P без сервера
3. **Устойчивость** — работает при блокировках, через email/BT/WiFi, даже без интернета
...
**Результат ИТЕРАЦИИ 3**: P2P + E2E с пост-квантовым шифрованием — **ВЫПОЛНЕНО**
...
### 4.2 Два раздела с разделителем

```
┌─────────────────────────────┐
│ [▼] КОНТАКТЫ  (12 онлайн)   │  ← приватное пространство
│  👤 Alice (482736)          │
│  👤 Bob   (193847)          │
...
- [x] 4.2 Два раздела списка контактов (Разделитель)
- [x] 4.3 Buzz! (Кнопка гудка + анимация + протокол)
- [/] 4.4 Отдельные окна чата (Electron / Virtual) — В ПРОЦЕССЕ (стабильность ядра проверена)

**Результат ИТЕРАЦИИ 4**: ICQ-дух, buzz, разделённые секции — **ВЫПОЛНЕНО (База заложена, исправляем P2P)**

### 🐞 Текущие блокировщики:
- Ошибка `simple-peer` в браузере при создании P2P соединения (`reading 'call'`).
- Требуется переключение на браузерную версию библиотеки (`simple-peer/simplepeer.min.js`).

### 📅 План на завтра:
1. Исправить инициализацию `simple-peer` (баг с `undefined 'call'`).
2. Проверить работу приватных сообщений (шифрование X25519 + Kyber768).
3. Добавить создание и вступление в группы (пункт 3.3).
4. Переход к виртуальным перетаскиваемым окнам.
