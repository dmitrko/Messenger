# NostaChat: Полный план разработки v2.1

> **СТАТУС ПРОЕКТА (01.04.2026):**
> ✅ **Авторизация**: Реализована система запросов (Add/Authorize/Block) с текстом приветствия.
> ✅ **Интерфейс**: Созданы Win9x диалоговые окна (вместо prompt), внедрен Drag-and-Drop для групп контактов.
> ✅ **Звуки**: Интеграция `Contact.wav` для авторизации.
> ✅ **База данных**: Миграция на `NostaChat_v5` для стабильности IndexedDB.
> ✅ **Шифрование**: Hybrid X25519 + Kyber-768 (Full E2EE).

> **Версия 2.1** — Актуализация социального слоя:
> Авторизация, блокировки, перетаскивание контактов, звуки запросов.
...
## ✅ ИТЕРАЦИЯ 5: История + Виртуальные окна + Звуки (3-4 дня)

### 5.1 Локальное хранилище (Dexie.js)
- [x] Создана схема БД (NostaChat_v5).
- [x] Персистентность сообщений и контактов.
- [x] Изоляция данных по UIN (Multi-account ready).

### 5.2 Window Manager & GUI
- [x] Система виртуальных окон (ChatWindow.tsx).
- [x] Авторизационные формы и диалоги (Win9x style).
- [x] Перетаскивание контактов между зонами (DnD).

### 5.3 Звуковое сопровождение
- [x] Интеграция ICQ звуков (Message, MsgSent, Auth, Global, ChatBeep, Contact).

**Результат ИТЕРАЦИИ 5**: Полный цикл обмена сообщениями с историей и авторизацией — **ВЫПОЛНЕНО**
...
- [ ] Серверный поиск в Map `onlineUsers` по нику (частичное совпадение).
- [ ] Вывод списка найденных UIN для быстрого добавления в Auth Request.

### 2. User Info (Get Info Window)
- [ ] Классическое окно с данными пользователя (UIN, Nick, Status).
- [ ] Возможность устанавливать локальное "имя контакта" (Alias).

### 3. Улучшение UX Чата
- [ ] Кликабельные UIN в сообщениях (автоматическое копирование или переход в чат).
